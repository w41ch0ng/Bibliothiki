...
<div id="readingListContainer" class="container mt-3">
  <div class="row">
    <% 
      List<Book> readingList = (List<Book>) request.getAttribute("readingList");
      if (readingList != null && !readingList.isEmpty()) {
        for (Book book : readingList) {
    %>
    <div class="col-md-3">
      <a href="bookDetails.jsp?id=<%= book.getID() %>">
        <img src="<%= book.getImageURL() %>" alt="<%= book.getTitle() %>" class="img-fluid book-cover">
      </a>
    </div>
    <div class="col-md-8">
      <h2><a href="Books?id=<%= book.getID() %>"><%= book.getTitle() %></a></h2>
      <p><strong>Author:</strong> <%= book.getAuthor() %></p>
    <form method="post" action="Remove">
    <input type="hidden" name="bookID" value="<%= book.getID() %>">
    <div class="form-group form-button">
        <button type="submit" class="form-submit delete-remove-button">Remove</button>
    </div>
</form>
    </div>
    <% 
    ...
    <div class="col">
      ...
      <div class="form-group form-button">
		<a href="Books" class="form-submit reading-list-browse-books">Browse Books</a>
	  </div>
    </div>
    <% } %>
  </div>
